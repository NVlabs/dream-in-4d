...
from threestudio.utils.lvdm_saving_utils import npz_to_video_grid
...
class SaverMixin:
    ...
    def create_loggers(self, cfg_loggers: DictConfig) -> None:
        if "wandb" in cfg_loggers.keys() and cfg_loggers.wandb.enable:
            # wandb.login(key=cfg_loggers.wandb.api_key)
            self._wandb_logger = WandbLogger(
                project=cfg_loggers.wandb.project, name=cfg_loggers.wandb.name, save_dir="../dream-in-4D-output/"
            )

            wandb.init(dir='../dream-in-4D-output/')
    ...
    def save_results(self, videos,
                     save_name="results", save_fps=12
                     ):
        save_dir = self.get_save_path('guidance_eval')
