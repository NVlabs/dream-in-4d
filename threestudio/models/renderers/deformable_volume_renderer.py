# ------------------------------------------------------------------------------
# Code is modified from https://github.com/threestudio-project/threestudio/blob/main/threestudio/models/renderers/nerf_volume_renderer.py
...
@threestudio.register("deformable-volume-renderer")
class DeformableVolumeRenderer(VolumeRenderer):
    @dataclass
    class Config(VolumeRenderer.Config):
        ...
        return_comp_displacement: bool = False
        grid_prune_num_time_step: int = 12
        grid_prune_update_n_step: int = 16
    ...
    def forward(
        ...
        **kwargs
        ...
        times_flatten: Float[Tensor, 'Nr 1'] = (
            times.reshape(-1, 1, 1, 1)
            .expand(-1, height, width, -1)
            .reshape(-1, 1)
        )
        ...
        def sigma_fn(t_starts, t_ends, ray_indices):
            ...
            t_times = times_flatten[ray_indices]
            if self.training:
                sigma = self.geometry.forward_density(positions, t_times)[..., 0]
            else:
                sigma = chunk_batch(
        ...
        if self.training:
            geo_out = self.geometry(
                positions, t_times, output_normal=self.material.requires_normal
            )
        ...
        if 'displacement' in geo_out and self.cfg.return_comp_displacement:
            comp_displacement: Float[Tensor, "Nr Nc"] = nerfacc.accumulate_along_rays(
                weights[..., 0], values=geo_out['displacement'], ray_indices=ray_indices, n_rays=n_rays
            )
        ...
        if bg_color is None:
            bg_color = comp_rgb_bg.reshape(-1, comp_rgb_bg.shape[-1])
        else:
        ...
        if 'displacement' in geo_out and self.cfg.return_comp_displacement:
            out["comp_displacement"] = comp_displacement.view(batch_size, height, width, -1)
        if self.training:
            out.update(
                {
                    "weights": weights,
            ...
            if "normal" in geo_out:
                ...
                if self.cfg.return_normal_perturb:
                    normal_perturb = self.geometry(
                        positions + torch.randn_like(positions) * 1e-2, t_times,
                        output_normal=self.material.requires_normal,
                    )["normal"]
    ...
    def update_step(
        ...
        if self.cfg.grid_prune:
            if self.geometry.__class__.__name__ in ["DeformableImplicitVolume"]:
                # get max voxel density across time.
                def occ_eval_fn(x):
                    density = []
                    for t in range(self.cfg.grid_prune_num_time_step):
                        density.append(self.geometry.forward_density(x, torch.ones_like(x[:, [0]]) * t / (self.cfg.grid_prune_num_time_step - 1)))

                    density = torch.amax(torch.stack(density, 0), 0)
            ...
            if self.training and not on_load_weights:
                self.estimator.update_every_n_steps(
                    step=global_step, occ_eval_fn=occ_eval_fn, n=self.cfg.grid_prune_update_n_step
                )
