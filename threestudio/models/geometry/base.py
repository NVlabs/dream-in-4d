# ------------------------------------------------------------------------------
# Code is modified from https://github.com/threestudio-project/threestudio/blob/main/threestudio/models/geometry/base.py
...
def contract_both_way(x: Float[Tensor, "... 3"], bbox1: Float[Tensor, "2 3"], bbox2: Float[Tensor, "2 3"]):
    x = scale_tensor(x, bbox1, bbox2)
    return x
...
class Base4DImplicitGeometry(BaseGeometry):
    @dataclass
    class Config(BaseGeometry.Config):
        radius: float = 1.0
    cfg: Config
